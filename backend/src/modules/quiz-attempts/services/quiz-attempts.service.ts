  ...
  async startAttempt(studentId: number, chapterId: number) {
    const accessRows = await this.dataSource.query(
      `SELECT ch.MaChuong AS maChuong,
       ...
       LEFT JOIN ChuongHoc previous
         ON previous.MaKH = ch.MaKH AND previous.ThuTu = (
           SELECT MAX(beforeChapter.ThuTu)
           FROM ChuongHoc beforeChapter
           WHERE beforeChapter.MaKH = ch.MaKH
             AND beforeChapter.ThuTu < ch.ThuTu
         )
  ...
  async getChapterAccess(studentId: number, chapterId: number) {
    const rows = await this.dataSource.query(
      `SELECT ch.MaChuong AS maChuong,
       ...
       LEFT JOIN ChuongHoc previous
         ON previous.MaKH = ch.MaKH AND previous.ThuTu = (
           SELECT MAX(beforeChapter.ThuTu) FROM ChuongHoc beforeChapter
