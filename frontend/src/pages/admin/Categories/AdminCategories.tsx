...
import { Layers3, Loader2, Pencil, Plus, Search, Trash2, XCircle } from 'lucide-react';
import AdminLayout from '../../../layouts/AdminLayout';
import Pagination from '../../../components/Pagination';
import { useAdminCategories, type AdminCategoryItem } from './hooks/useAdminCategories';
        ...
        <AdminLayout>
                ...
                <section className="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm">
                    <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-[16px] font-semibold text-slate-800">Danh sách danh mục</h2>
                        </div>
                                ...
                                <table className="min-w-full table-fixed text-left">
                                    ...
                                    <tbody className="divide-y divide-slate-100">
                                        {visibleCategories.map((category) => (
                                            <tr key={category.maDM} className="transition hover:bg-slate-50/60">
                                                ...
                                                <td className="px-6 py-5">
                                                    <p className="line-clamp-2 max-w-[600px] text-[14px] leading-6 text-slate-600">
                                                        {category.moTa?.trim() || 'Không có mô tả cho danh mục này.'}
                                                    </p>
                                                </td>
            ...
            {formOpen ? (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm" onClick={() => !submitting && closeForm()} />
                        ...
                        <form onSubmit={handleSubmit} className="mt-6 space-y-5">
                            <div>
                                ...
                                <input
                                    value={form.TenDM}
                                    onChange={(event) =>
                                        setForm((current) => ({ ...current, TenDM: event.target.value }))
                                    }
                            ...
                            <div>
                                ...
                                <textarea
                                    value={form.MoTa}
                                    onChange={(event) =>
                                        setForm((current) => ({ ...current, MoTa: event.target.value }))
                                    }
                                    rows={5}
            ...
            {deleteTarget ? (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm" onClick={() => !deleting && setDeleteTarget(null)} />
                    <div className="relative w-full max-w-md rounded-[28px] border border-slate-200 bg-white p-6 shadow-2xl">
                        ...
                        <p className="mt-2 text-center text-[14px] leading-6 text-slate-500">
                            ...
                            <span className="font-semibold text-slate-800">"{deleteTarget.tenDM}"</span>?
