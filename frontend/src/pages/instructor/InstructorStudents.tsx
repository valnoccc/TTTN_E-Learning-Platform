      ...
      <div className="space-y-6">
          ...
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl">
              ...
              <p className="mt-3 max-w-xl text-sm leading-6 text-slate-500 sm:text-[15px]">
                Theo dõi danh sách học viên, xem thông tin mua sắm và lọc theo tiêu chí tùy chọn.
              </p>
            </div>
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          <div className="grid gap-3 lg:grid-cols-[minmax(0,1.4fr)_minmax(220px,0.8fr)_auto]">
            <label className="flex items-center gap-3 rounded-md border border-slate-200 bg-slate-50 px-4 py-3 transition focus-within:border-emerald-500 focus-within:bg-white">
            ...
            <select
              ...
              className="rounded-md border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-800 outline-none transition focus:border-emerald-500 focus:bg-white"
            >
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-md bg-slate-900 px-5 py-3 text-sm font-medium text-white shadow-sm transition hover:bg-black"
            >
              <RefreshCw size={16} />
              Lọc dữ liệu
            </button>
        ...
        <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
          <div className="flex flex-col gap-2 border-b border-slate-100 px-6 py-5 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-500">
                {board.students.length} học viên đang hiển thị theo bộ lọc.
              </p>
            </div>
          </div>
          ...
          {loading ? (
            <div className="space-y-4 p-6">
              {[1, 2, 3].map((row) => (
                <div key={row} className="rounded-md border border-slate-100 bg-slate-50/60 p-5 animate-pulse h-24" />
              ))}
            </div>
            ...
            <div className="flex min-h-[320px] flex-col items-center justify-center px-6 text-center">
              ...
              <h3 className="mt-5 text-lg font-bold text-slate-800">Chưa có học viên phù hợp</h3>
            </div>
          ) : (
                  ...
                  <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                      <div className="flex items-center gap-4">
                        <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-md bg-slate-800 text-sm font-semibold text-white shadow-sm">
                          {getStudentInitials(student.studentName)}
                        </div>
                        <div className="min-w-0">
                          <h3 className="truncate text-base font-bold text-slate-800">{student.studentName}</h3>
...
// Giữ cấu trúc component con nhưng đổi phong cách thiết kế phẳng
function StatCard({ icon, label, value, wide = false }: { icon: ReactNode; label: string; value: string | number; wide?: boolean; }) {
  return (
    <div className={`rounded-md border border-slate-200 bg-white p-4 shadow-sm ${wide ? 'sm:col-span-2' : ''}`}>
      ...
      <div className="mt-3 text-lg font-bold tracking-tight text-slate-800">{value}</div>
    </div>
  );
}
