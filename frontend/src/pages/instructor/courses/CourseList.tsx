...
export default function InstructorCourses() {
  const { courses, loading, handleDelete, handleToggleStatus, handleArchive } =
    useCourseList();
      ...
      <div className="space-y-6">
        <div className="mb-5 flex items-center justify-between">
          ...
          <Link
            ...
            onMouseOver={(e) =>
              (e.currentTarget.style.backgroundColor = "#169b5c")
            }
            onMouseOut={(e) =>
          ...
          {loading ? (
                            ...
                            <button
                              onClick={() =>
                                setConfirmModal({
                                  ...
                                  type: "toggle",
                                  courseId: course.id,
                                  currentStatus: course.trang_thai,
                                })
                              }
            ...
            <div className="py-16 text-center">
              ...
              <p className="mb-6 mt-2 text-sm text-slate-500">
                Bạn chưa tạo khóa học nào trên hệ thống. Hãy bắt đầu với một bản
                nháp mới.
              </p>
      ...
      <ConfirmModal
        ...
        message={
          confirmModal.type === "delete"
            ...
            : confirmModal.type === "archive"
              ? "Khóa học sẽ không bán cho học viên mới, nhưng học viên đã mua vẫn tiếp tục học. Bạn vẫn có thể chỉnh sửa và gửi duyệt lại sau. Dữ liệu khóa học vẫn được giữ lại."
              : "Khóa học sẽ bị ẩn hoàn toàn, không thể bán và không cho học viên truy cập. Bạn có chắc chắn muốn tạm ẩn?"
        }
        confirmText={
          confirmModal.type === "delete"
            ? "Xóa khóa học"
        ...
        isDestructive={
          confirmModal.type === "delete" || confirmModal.type === "archive"
        }
...
function StatusBadge({ status }: { status: string }) {
  ...
  switch (status) {
    ...
    case "HIDDEN":
      return (
        <span
          className={`${baseClass} border-slate-200 bg-slate-100 text-slate-600`}
        >
          Đang ẩn
        </span>
      );
    ...
    case "REJECTED":
      return (
        <span
          className={`${baseClass} border-rose-300 bg-rose-50 text-rose-700`}
        >
          Bị từ chối
        </span>
      );
