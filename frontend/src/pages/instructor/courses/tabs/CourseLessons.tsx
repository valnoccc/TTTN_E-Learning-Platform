      ...
      <aside className="h-fit border border-slate-300 bg-white p-4 xl:sticky xl:top-5">
        ...
        {loading ? (
          <div className="space-y-2" aria-label="Đang tải chương học">
            {[1, 2, 3].map((item) => (
              <div key={item} className="h-12 animate-pulse bg-slate-100" />
            ))}
          </div>
        ) : chapters.length === 0 ? (
            ...
            {chapters.map((chapter, chapterIndex) => {
              ...
              return (
                <div
                  ...
                  className="overflow-hidden border border-slate-400 bg-white"
                >
                  <div
                  ...
                  {isExpanded ? (
                    <div className="bg-white">
                      ...
                      {chapter.baiHocs.length === 0 ? (
                        <p className="border-t border-slate-200 px-3 py-3 text-xs italic text-slate-500">
                          Chưa có bài học.
                        </p>
                      ) : null}
                      {!isLocked ? (
                        activeAddLessonChapterId === chapter.maChuong ? (
                          <div className="border-t border-slate-200 bg-slate-50 p-3">
                            ...
                            <div className="mt-2 flex justify-end gap-2">
                              ...
                              <button
                                type="button"
                                onClick={async () => {
                                  const createdLesson = await handleAddLesson(
                                    chapter.maChuong,
                                  );
                                  if (createdLesson) {
                          ...
                          <button
                            type="button"
                            onClick={() => {
                              setActiveAddLessonChapterId(chapter.maChuong);
                              setNewLessonTitle("");
                            }}
        ...
        <div className="flex flex-col gap-4 border-b border-slate-300 pb-5 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="mb-2 text-sm text-slate-500">
              {selectedChapter?.tenChuong || "Chương học"} /{" "}
              {selectedLesson?.tenBaiHoc || "Chưa chọn bài học"}
            </p>
          ...
          <div className="flex flex-wrap gap-2">
            {!isLocked && selectedLesson ? (
              <button
                type="button"
                onClick={() => void handleDeleteLesson(selectedLesson.maBH)}
                className="rounded-sm border border-slate-800 bg-white px-4 py-2 text-sm font-bold text-slate-900 hover:bg-slate-100"
              >
                Xóa bài học
              </button>
            ) : null}
            {!isLocked && selectedLesson ? (
              <button
                ...
                className="rounded-sm bg-slate-800 px-4 py-2 text-sm font-bold text-white hover:bg-slate-700 disabled:cursor-not-allowed disabled:opacity-50"
              >
        ...
        {selectedLesson ? (
            ...
            <div>
              ...
              {videoPreview ? (
                <div className="space-y-3 border border-slate-300 bg-slate-50 p-3">
                  ...
                  {!isLocked ? (
                    <button
                      type="button"
                      onClick={() =>
                        document.getElementById("inline-video-upload")?.click()
                      }
                      className="rounded-sm border border-slate-700 bg-white px-3 py-2 text-sm font-bold text-slate-800 hover:bg-slate-100"
                    >
                      Thay video
                    </button>
                  ) : null}
                ...
                <button
                  ...
                  className="flex min-h-40 w-full flex-col items-center justify-center border border-dashed border-slate-500 bg-slate-50 px-5 text-center hover:bg-slate-100"
                >
                  <PlayCircle size={30} className="mb-2 text-slate-700" />
                  <span className="font-bold text-slate-900">Upload video</span>
                  <span className="mt-1 text-sm text-slate-500">
                    Định dạng video bài giảng
                  </span>
                </button>
              )}
            </div>
            <div className="grid gap-5 sm:grid-cols-2">
              <div>
                ...
                <input
                  ...
                  onChange={(event) =>
                    handleLessonFieldChange("thu_tu", event.target.value)
                  }
                  disabled={isLocked}
                  className="w-full rounded-sm border border-slate-400 bg-white px-4 py-3 text-base outline-none focus:border-[#1dbf73] disabled:bg-slate-50"
                />
              </div>
          ...
          <div className="mt-6 border border-dashed border-slate-400 bg-slate-50 px-6 py-16 text-center">
            ...
            <p className="mt-2 text-sm text-slate-500">
              Chọn một bài học ở cột bên trái hoặc tạo bài học mới.
            </p>
          </div>
        )}
      </section>
