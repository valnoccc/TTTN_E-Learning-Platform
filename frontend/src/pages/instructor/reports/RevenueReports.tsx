    ...
    <div className="mx-auto max-w-7xl space-y-6 animate-fade-in text-slate-800">
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <p className="text-[13px] font-semibold text-slate-500">
              Thực nhận ({range === "this_month" ? "Tháng này" : "Kỳ này"})
            </p>
          ...
          <div className="mt-2 flex items-center justify-between text-[12px] font-medium">
            <div
              className={`flex items-center gap-1 ${board.overview.revenueGrowth >= 0 ? "text-emerald-600" : "text-rose-600"}`}
            >
              <svg
              ...
              >
                <path
                  ...
                  strokeLinejoin="round"
                  d={
                    board.overview.revenueGrowth >= 0
              ...
              <span>
                {board.overview.revenueGrowth >= 0 ? "+" : ""}
                {board.overview.revenueGrowth}% so với kỳ trước
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-4 text-2xl font-black text-slate-900">
            {board.learning.repeatStudents}
          </p>
          <div className="mt-2 flex items-center justify-between text-[12px] font-medium text-slate-500">
            <span>Học viên đã mua từ 2 khóa học trở lên</span>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-4 text-2xl font-black text-slate-900">
            {board.learning.completionRate !== null
              ? `${board.learning.completionRate}%`
              : "0%"}
          </p>
          <div className="mt-2 flex items-center justify-between text-[12px] font-medium text-slate-500">
            <span>
              {board.learning.completionRateLabel ||
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[12px] text-slate-500">
                    Học viên đang đợi giải đáp
                  </p>
                </div>
              </div>
              <span
                className={`rounded-full px-2.5 py-0.5 text-[12px] font-bold ${board.operations.unansweredQuestions > 0 ? "bg-rose-100 text-rose-600" : "bg-slate-100 text-slate-600"}`}
              >
            ...
            <div className="flex items-center justify-between p-3 hover:bg-slate-50 rounded-lg transition cursor-pointer">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[12px] text-slate-500">
                    Nên cảm ơn hoặc giải thích cho HV
                  </p>
                </div>
              </div>
              <span
                className={`rounded-full px-2.5 py-0.5 text-[12px] font-bold ${board.operations.unrespondedReviews > 0 ? "bg-amber-100 text-amber-700" : "bg-slate-100 text-slate-600"}`}
              >
            ...
            {adminAlerts.length > 0 ? (
              adminAlerts.map((notification) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`mt-0.5 shrink-0 rounded-full p-1.5 ${tone.badge}`}
                    >
                      {tone.icon}
                    </div>
              ...
              <div className="rounded-xl border border-blue-100 bg-white p-3 flex items-start gap-3">
                ...
                <div>
                  <p className="text-[13px] font-bold text-slate-900">
                    Chưa có cảnh báo từ admin
                  </p>
                  <p className="mt-0.5 text-[12px] text-slate-500">
                    Hiện chưa có thông báo kiểm duyệt hoặc từ chối khóa học nào
                    cần xử lý.
                  </p>
                </div>
              </div>
                  ...
                  <PieChart>
                    ...
                    <Tooltip
                      formatter={(value: number, _name: string, item: any) => [
                        `${value} đơn | ${formatCurrency(Number(item.payload.grossRevenue))}`,
                        item.payload.label,
                      ]}
                    />
                  </PieChart>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm flex flex-col">
          <div className="border-b border-slate-100 bg-white px-6 py-5 flex-shrink-0">
            <h2 className="text-[15px] font-bold text-slate-900">
              Phân bổ đánh giá (Chất lượng)
            </h2>
            <p className="mt-1 text-[12px] font-medium text-slate-500">
              Biểu đồ phân bổ tỷ lệ đánh giá sao từ học viên.
            </p>
          </div>
          ...
          <div className="flex flex-col md:flex-row md:h-[300px] items-center px-6 py-6 gap-6 md:gap-0">
            ...
            <div className="w-full md:w-1/3 text-center md:border-r border-slate-100 md:pr-4">
              <p className="text-5xl font-black text-slate-900">
                {board.quality.averageRating?.toFixed(1) || "--"}
              </p>
              <div className="flex justify-center mt-2 text-amber-500">
                {[...Array(5)].map((_, i) => (
                  <StarIcon
                    ...
                    className="h-5 w-5"
                    fill={
                      i < Math.round(board.quality.averageRating || 0)
                        ? "currentColor"
                        : "none"
                    }
                  />
                    ...
                    >
                      {ratingData.map((entry, index) => {
                        ...
                        return (
                          <Cell
                            key={`cell-${index}`}
                            fill={colors[entry.rating] || "#f59e0b"}
                          />
                        );
                      })}
                    </Bar>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm flex flex-col">
          <div className="border-b border-slate-100 bg-white px-6 py-5 flex-shrink-0">
            ...
            <p className="mt-1 text-[12px] font-medium text-slate-500">
              Những khóa học mang lại trải nghiệm tốt nhất cho học viên.
            </p>
          </div>
              ...
              board.quality.topRatedCourses.map((course) => (
                ...
                >
                  <div className="flex-1 min-w-0 pr-4">
                    <p className="truncate font-bold text-slate-900 text-[13px]">
                      {course.courseName}
                    </p>
        ...
        <div className="flex items-center justify-between border-b border-slate-100 bg-white px-6 py-5">
          <div>
            <h2 className="text-[16px] font-bold text-slate-900">
              Phân tích chuyên sâu khóa học
            </h2>
            <p className="mt-1 text-[13px] text-slate-500">
              Theo dõi chuyển đổi, hành vi học tập và rating của từng khóa.
            </p>
          </div>
        </div>
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left text-[13px] text-slate-600 min-w-[900px]">
            <thead className="bg-slate-50/80 border-b border-slate-100 text-[11px] font-bold uppercase tracking-wider text-slate-500">
              <tr>
                <th className="p-4 whitespace-nowrap w-[25%]">
                  Khóa học & Xếp hạng
                </th>
                ...
                <th className="p-4 whitespace-nowrap text-right">
                  Doanh Thu
                  <br />
                  <span className="text-[9px] font-medium normal-case text-emerald-500">
                    (Thực nhận)
                  </span>
                </th>
              </tr>
              ...
              {board.topCourses.length === 0 ? (
                <tr>
                  <td colSpan={5} className="p-8 text-center text-slate-500">
                    Chưa có dữ liệu khóa học trong kỳ.
                  </td>
                </tr>
              ) : (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-3">
                          <div className="h-10 w-16 shrink-0 overflow-hidden rounded bg-slate-200">
                            {course.imageUrl ? (
                              ...
                              <img
                                src={`https://ui-avatars.com/api/?name=${course.courseName}&background=f1f5f9&color=64748b`}
                                alt="Thumbnail"
                                className="h-full w-full object-cover"
                              />
                            )}
                          ...
                          <div>
                            ...
                            {index === 0 && (
                              <p className="text-[11px] font-semibold text-amber-500 uppercase tracking-wide mt-0.5">
                                Top 1 Doanh thu
                              </p>
                            )}
                            {index === 1 && (
                              <p className="text-[11px] font-semibold text-blue-500 uppercase tracking-wide mt-0.5">
                                Top 2 Bán chạy
                              </p>
                            )}
                          </div>
                      ...
                      <td className="p-4 align-middle">
                        <div className="w-full max-w-[120px] mx-auto space-y-1.5">
                          ...
                          <div className="h-1.5 w-full rounded-full bg-slate-100 overflow-hidden">
                            <div
                              className={`h-full rounded-full ${mockCompletion > 50 ? "bg-emerald-500" : "bg-amber-500"}`}
                      ...
                      <td className="p-4 align-middle text-right">
                        ...
                        <p className="text-[10px] text-slate-400 mt-0.5">
                          Tổng: {formatCompactCurrency(course.grossRevenue)}
                        </p>
                      </td>
                    </tr>
                  );
