...
const SwiperComponent =
  Swiper && typeof Swiper === "object" && "default" in Swiper
    ? (Swiper as any).default
    : Swiper;
...
const renderRatingStars = (ratingValue: number) => {
  ...
  for (let star = 1; star <= 5; star += 1) {
    if (ratingValue >= star) {
      stars.push(
        <li key={star} className="list-inline-item">
...
const InstructorDetails = () => {
  ...
  const imgUrl = instructor.personImage?.startsWith("http")
    ? instructor.personImage
    : `/assets/images/${instructor.personImage}`;
            ...
            <Row>
              <Col md="4">
                <div className="instructor-img">
                  <img
                    src={imgUrl}
                    alt={instructor.personName}
                    className="img-fluid"
                  />
                  <ul className="list-unstyled getintouch">
                    {instructor.phone && (
                      <li>
                        <i className="las la-phone"></i> {instructor.phone}
                      </li>
                    )}
                    {instructor.email && (
                      <li>
                        <i className="lar la-envelope"></i> {instructor.email}
                      </li>
                    )}
                  </ul>
              ...
              <Col md="8">
                ...
                <div className="specialization-box">
                  <h5>Chuyên môn</h5>
                  <p>{instructor.personTitle || "Chưa cập nhật chuyên môn."}</p>
                </div>
                <div className="qual-expe d-flex">
                    ...
                    {instructor.qualifications?.length ? (
                      instructor.qualifications.map(
                        (qualification: any, index: number) => (
                          <div
                            className="qual-expe-box"
                            key={qualification.MaBangCap || index}
                          >
                            <h6>{qualification.TenTruong}</h6>
                            <p>
                              {qualification.TenBangCap}
                              {qualification.ChuyenNganh
                                ? ` · ${qualification.ChuyenNganh}`
                                : ""}
                            </p>
                            {(qualification.NamBatDau ||
                              qualification.NamKetThuc) && (
                              <small>
                                {qualification.NamBatDau || "—"} -{" "}
                                {qualification.NamKetThuc || "Nay"}
                              </small>
                            )}
                          </div>
                        ),
                      ...
                      <p className="empty-profile-detail">
                  ...
                  <div className="experiance">
                    ...
                    {instructor.experiences?.length ? (
                      instructor.experiences.map(
                        (experience: any, index: number) => (
                          <div
                            className="qual-expe-box"
                            key={experience.MaKinhNghiem || index}
                          >
                            <h6>{experience.TenDonVi}</h6>
                            <p>{experience.ChucVu}</p>
                            <small>
                              {experience.NamBatDau || "—"} -{" "}
                              {experience.DangLamViec
                                ? "Nay"
                                : experience.NamKetThuc || "—"}
                            </small>
                            {experience.MoTa && (
