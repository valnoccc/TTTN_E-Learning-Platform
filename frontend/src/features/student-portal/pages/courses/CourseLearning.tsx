...
const saveCurrentLessonSilently = async (
  courseId: string,
  lessonId: number,
) => {
  try {
    await axiosClient.patch(`/users/me/courses/${courseId}/current-lesson`, {
      lessonId,
    });
    console.log(
      `[CourseLearning] Đã lưu bài học gần nhất: courseId=${courseId}, lessonId=${lessonId}`,
    );
  } catch (err) {
    // Fail-safe: không hiện lỗi cho người dùng
    console.warn(
      "[CourseLearning] Không thể lưu bài học gần nhất (silent fail):",
      err,
    );
  }
};
...
export default function CourseLearning() {
  ...
  const [courseName, setCourseName] = useState<string>(
    `Không gian học tập (Khóa học: ${id})`,
  );
  ...
  const [showCertificatePopup, setShowCertificatePopup] =
      ...
      try {
        ...
        const [courseRes, curriculumRes, progressRes, completedLessonsRes] =
          await Promise.all([
            ...
            userId
              ? axiosClient.get(`/users/me/courses`).catch((e) => {
                  console.error(">>> [API Error] Lỗi tải my courses:", e);
                  return null;
                })
              : Promise.resolve([]),
            userId
              ? axiosClient.get(`/users/me/progress`).catch((e) => {
        ...
        const extractArray = (res: any): any[] => {
          ...
          if (res.data && res.data.data && Array.isArray(res.data.data)) {
            console.log(
              ">>> [FIXED] Đã tìm thấy mảng tại res.data.data:",
              res.data.data,
            );
        ...
        if (finalCurriculum.length > 0) {
          ...
          void Promise.all(
            data.map((module: any) =>
              chapterQuiz.getChapterAccess(module.maChuong),
            ),
          );
        }
        ...
        if (userId && builtCurriculum.length > 0) {
          try {
            ...
            const lastLessonRes: any = await axiosClient.get(
              `/users/me/courses/${id}/current-lesson`,
            );

            console.log(
              ">>> Dữ liệu bài học gần nhất bốc từ NestJS:",
              lastLessonRes,
            );
            ...
            console.warn(
              "[CourseLearning] Không lấy được bài học gần nhất (fail-safe):",
              err,
            );
          }
        }
        ...
        if (lastLessonId && builtCurriculum.length > 0) {
          for (const module of builtCurriculum) {
            ...
            if (foundLesson) {
              console.log(
                ">>> Khôi phục thành công bài học cũ ID:",
                lastLessonId,
                "| Tên:",
                foundLesson.tenBaiHoc,
                "| Chương:",
                module.tenChuong,
              );
              ...
              const isFirstLesson =
                Number(builtCurriculum[0]?.baiHocs?.[0]?.maBH) ===
                Number(lastLessonId);
              if (!isFirstLesson) {
          ...
          if (!restored) {
            console.warn(
              ">>> lastLessonId",
              lastLessonId,
              "không tìm thấy trong curriculum – fallback bài đầu",
            );
          }
        }
        ...
        if (!restored && builtCurriculum.length > 0) {
          ...
          if (
            builtCurriculum[0].baiHocs &&
            builtCurriculum[0].baiHocs.length > 0
          ) {
        ...
        // ─── Cập nhật progress % ──────────────────────────────────────────────
        const myCourses = Array.isArray(progressRes)
          ? progressRes
  ...
  const activeLessonIndex = allLessons.findIndex(
    (l: any) => l.maBH === activeLesson?.maBH,
  );
  const nextLesson =
  ...
  const handleLessonClick = async (lesson: any) => {
    if (chapterQuiz.activeAttempt && !chapterQuiz.result) {
      toast.error(
        "Vui lòng hoàn thành và nộp bài kiểm tra trước khi chuyển đi.",
      );
      return;
    }
    if (chapterQuiz.activeAttempt && chapterQuiz.result) {
    ...
    const activeModule = activeLesson
      ? curriculum.find((m: any) =>
          m.baiHocs?.some((l: any) => l.maBH === activeLesson.maBH),
        )
      : null;
    ...
    if (parentModule && isChangingChapter) {
      ...
      if (!access.canAccess) {
        ...
        if (!openedQuiz) {
          toast.error(
            "Bạn cần vượt qua bài kiểm tra chương hiện tại để tiếp tục.",
          );
        }
        return;
      }
  ...
  const handleNextLesson = async () => {
    ...
    const currentModule = activeLesson
      ? curriculum.find((module: any) =>
          module.baiHocs?.some(
            (lesson: any) => lesson.maBH === activeLesson.maBH,
          ),
        )
      : null;
    const nextModule = curriculum.find((module: any) =>
      module.baiHocs?.some((lesson: any) => lesson.maBH === nextLesson.maBH),
    );
  ...
  const hasCompletedAllLessons = (module: any) => {
    ...
    return (
      lessons.length > 0 && lessons.every((lesson: any) => lesson.completed)
    );
  };
    ...
    >
      ...
      <div className="h-16 bg-slate-950 border-b border-slate-800 flex items-center px-4 shrink-0 justify-between z-20 sticky top-0">
        <div className="flex items-center gap-4">
          ...
          <h1 className="font-semibold text-white line-clamp-1 hidden md:block">
            {courseName}
          </h1>
        </div>
                ...
                <div className="absolute right-0 top-12 w-64 bg-white rounded-2xl shadow-2xl border border-gray-100 p-4 z-50">
                  ...
                  <p className="text-xs text-slate-500 mb-4 leading-relaxed">
                    ...
                    <span className="font-semibold text-slate-700">
                      {
                        curriculum
                          .flatMap((m: any) => m.baiHocs ?? [])
                          .filter((l: any) => l.completed).length
                      }
                    </span>
                    /{curriculum.flatMap((m: any) => m.baiHocs ?? []).length}{" "}
                    bài.
                  </p>
      ...
      <div
        className="flex-grow flex flex-col lg:flex-row bg-white"
      ...
      >
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="w-full bg-black aspect-video lg:max-h-[70vh] relative shrink-0 overflow-hidden">
            {chapterQuiz.activeAttempt ? (
              <ChapterQuiz
                ...
                onRetry={() =>
                  void chapterQuiz.startQuiz(
                    chapterQuiz.activeAttempt!.chapterId,
                  )
                }
              ...
              <iframe
                src={`https://www.youtube.com/embed/${
                  activeLesson.videoUrl.includes("youtu.be/")
                    ...
                    : (new URLSearchParams(
                        new URL(activeLesson.videoUrl).search,
                      ).get("v") ?? "")
                }?rel=0`}
              ...
              <VideoPlaceholder
                message={
                  activeLesson ? "Tài liệu học tập" : "Chọn bài học để bắt đầu"
                }
              />
            )}
          </div>
          ...
          <div className="p-4 sm:p-6 md:p-10 max-w-5xl mx-auto w-full flex-1 pb-24">
            {activeTab === "overview" && (
              <CourseOverview courseData={courseData} curriculum={curriculum} />
            )}
            {activeTab === "qa" && (
              <CourseQA courseId={id || ""} currentLesson={activeLesson} />
            )}
            ...
            <div
              className="w-full h-40 clear-both"
              style={{ height: "160px" }}
            ></div>
          </div>
        </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar pb-10">
            ...
            <div className="bg-slate-50">
              {curriculum && curriculum.length > 0 ? (
                  ...
                  >
                    ...
                    >
                      ...
                      <span className="text-xs text-slate-500 font-medium whitespace-nowrap ml-2">
                        0/{module.baiHocs?.length || 0}
                      </span>
                    </button>
                    {expandedModules.includes(module.maChuong) &&
                      module.baiHocs && (
                        <div className="bg-slate-50 p-2 space-y-1 border-t border-slate-100">
                          {module.baiHocs.map((lesson: any) => (
                            ...
                            >
                              ...
                              <div className="flex-1">
                                <p
                                  className={`text-sm ${activeLesson?.maBH === lesson.maBH ? "text-emerald-600 font-bold" : "text-slate-700 font-medium"}`}
                                >
                                  {lesson.tenBaiHoc}
                                </p>
                          ...
                          {chapterQuiz.accessByChapter[module.maChuong]?.hasQuiz && <button
                            type="button"
                            onClick={() => {
                              if (!hasCompletedAllLessons(module)) {
                                toast.error(
                                  "Hãy xem và hoàn thành tất cả video trong chương trước.",
                                );
                                return;
                              }
                          ...
                          >
                            {chapterQuiz.accessByChapter[module.maChuong]
                              ?.quizPassed ? (
                              <CheckCircle
                                size={17}
                            ...
                            <span className="flex-1">
                              <span
                                className={`block text-sm font-bold ${chapterQuiz.accessByChapter[module.maChuong]?.quizPassed ? "text-emerald-700" : hasCompletedAllLessons(module) ? "text-amber-800" : "text-slate-600"}`}
                              >
                                Bài kiểm tra
                              </span>
                              <span
                                className={`mt-1 block text-xs ${hasCompletedAllLessons(module) ? "text-amber-700" : "text-slate-500"}`}
                              >
                                {chapterQuiz.accessByChapter[module.maChuong]
                                  ?.quizPassed
                                  ? "Đã hoàn thành"
                                  : hasCompletedAllLessons(module)
                                    ? ""
                                    : "Hoàn thành tất cả video để mở bài kiểm tra"}
                              </span>
                            </span>
                          </button>}
                        </div>
                ...
                <p className="text-center p-3 text-sm text-slate-500">
                  Đang tải danh sách bài học hoặc dữ liệu trống...
                </p>
              )}
            </div>
          </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-sm">
                    Hệ thống đang khởi tạo chứng chỉ cho bạn...
                  </p>
                </div>
              </div>
