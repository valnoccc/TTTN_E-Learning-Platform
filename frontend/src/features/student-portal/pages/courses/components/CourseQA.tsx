...
const UserAvatar = ({
  ...
  if (src && src.trim() !== "") {
    ...
    return (
      <div
        className={`shrink-0 rounded-full overflow-hidden bg-slate-200 border border-slate-200 ${sizeClasses[size]}`}
      >
        <img
          src={imgSrc}
          alt={name}
  ...
  return (
    <div
      className={`shrink-0 rounded-full flex items-center justify-center font-bold text-white uppercase bg-slate-800 ${sizeClasses[size]}`}
    >
      {name ? name.charAt(0) : "U"}
    </div>
  );
...
function getAvatarUrl(user: any) {
  return (
    user?.avatarUrl || user?.avatar || user?.photoUrl || user?.imageUrl || ""
  );
}
...
export default function CourseQA({
  ...
  const fetchDiscussions = useCallback(async () => {
    try {
      ...
      if (selectedThread) {
        const updatedThread = parsedData.find(
          (t: any) => t.discussionId === selectedThread.discussionId,
        );
        if (updatedThread) setSelectedThread(updatedThread);
      }
    } catch (err: any) {
      console.error(
        "[CourseQA] fetchDiscussions error:",
        err?.response?.status,
        err?.message,
      );
      setDiscussions([]);
  ...
  const handlePostQuestion = async () => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.message || "Có lỗi xảy ra khi đăng câu hỏi",
      );
    } finally {
      setIsPosting(false);
    }
  ...
  const handleToggleLike = async (
    ...
    try {
      const res: any = await axiosClient.post(
        `/public/courses/${courseId}/discussions/${discussionId}/toggle-like`,
      );
      ...
      setDiscussions((prev) =>
        prev.map((d) =>
          d.discussionId === discussionId ? updateThread(d) : d,
        ),
      );
  ...
  const handleHideDiscussion = async (
    ...
    try {
      await axiosClient.patch(
        `/public/courses/${courseId}/discussions/${discussionId}/hide`,
      );
      toast.success("Đã xử lý bình luận!");
      // Xóa khỏi danh sách hiện tại (optimistic)
      setDiscussions((prev) =>
        prev.filter((d) => d.discussionId !== discussionId),
      );
  ...
  const handleDeleteDiscussion = async (
    ...
    try {
      await axiosClient.delete(
        `/public/courses/${courseId}/discussions/${discussionId}`,
      );
      toast.success("Đã xử lý bình luận!");
      setDiscussions((prev) =>
        prev.filter((d) => d.discussionId !== discussionId),
  ...
  const DiscussionMenu = ({
    ...
    const currentUserId =
      currentUser?.maND || currentUser?.id || currentUser?.sub;
    const currentRole = (
      currentUser?.vaiTro ||
      currentUser?.role ||
      ""
            ...
            {isAuthor && (
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  setOpen(false);
                  handleDeleteDiscussion(courseIdStr, disc.discussionId);
                }}
              ...
              <button
                onClick={(e) => {
                  ...
                  setReportModal({
                    ...
                    reportedUserId: disc.userId,
                    reportedUserName: disc.userName,
                  });
                }}
              ...
              <>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    setOpen(false);
                    handleHideDiscussion(courseIdStr, disc.discussionId);
                  }}
              ...
              <div className="flex items-start justify-between gap-4 mb-1">
                ...
                <div className="flex items-center gap-1.5 text-slate-500 shrink-0">
                  <span className="font-semibold text-sm">
                    {selectedThread.upvotes || 0}
                  </span>
                  <button
                    onClick={(e) =>
                      handleToggleLike(e, selectedThread.discussionId)
                    }
                  ...
                  >
                    <ThumbsUp
                      size={16}
                      className={
                        selectedThread.likedUserIds?.includes(
                          currentUser?.maND ||
                            currentUser?.id ||
                            currentUser?.sub,
                        )
