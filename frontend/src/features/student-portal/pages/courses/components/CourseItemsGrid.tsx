      ...
      if (userString) {
        ...
        if (userId) {
          try {
            ...
            setPurchasedCourses(
              userCourses.map((c: any) => Number(c.MaKH || c.maKH || c.id)),
            );
          } catch (e) {
            ...
            <div className="group relative w-full rounded-2xl border border-gray-100 bg-white transition-all hover:shadow-xl">
              ...
              <div className="pointer-events-none absolute inset-0 z-50 hidden opacity-0 transition-all duration-300 group-hover:pointer-events-auto group-hover:opacity-100 xl:block">
                <div className="flex h-full w-full flex-col justify-center rounded-2xl border border-gray-100 bg-white p-6 shadow-2xl">
                  ...
                  <h5 className="mb-2 text-xl font-bold text-slate-800">
                    {data.tenKhoaHoc}
                  </h5>
                  <div className="mb-3 flex items-center text-sm text-gray-500">
                    ...
                    <span>
                      <i className="las la-signal mr-1"></i>Mọi cấp độ
                    </span>
                  </div>
                  ...
                  <ul className="mb-6 space-y-2 text-sm text-gray-600">
                    <li className="flex items-start">
                      <i className="las la-check text-emerald-500 mr-2 mt-0.5"></i>{" "}
                      {Number(data.totalLessons ?? data.lessonCount ?? 0)} bài học &amp;{" "}
                      {Number(data.totalQuestions ?? 0)} câu hỏi trắc nghiệm
                    </li>
                    <li className="flex items-start">
                      <i className="las la-check text-emerald-500 mr-2 mt-0.5"></i>{" "}
                      Hỗ trợ giải đáp code trực tiếp (Q&amp;A)
                    </li>
                    <li className="flex items-start">
                      <i className="las la-check text-emerald-500 mr-2 mt-0.5"></i>{" "}
                      Sở hữu trọn đời, cập nhật miễn phí
                    </li>
                  </ul>
                  <div className="flex gap-2">
                    <Link
                      to={courseUrl}
                      className="flex-1 rounded-lg bg-emerald-500 py-2.5 text-center text-sm font-medium text-white transition-colors hover:bg-emerald-600 hover:text-white text-decoration-none hover:no-underline"
                    >
                    ...
                    {purchasedCourses.includes(Number(data.maKH)) ? (
                      <span className="text-muted small self-center ml-2">
                        Đã sở hữu
                      </span>
                    ) : (
                      <button
                    ...
                    >
                      <i
                        className={`${wishlistItems.some((item) => item.id === data.maKH) ? "las la-heart" : "lar la-heart"} text-xl`}
                      ></i>
                    </button>
                  </div>
                </div>
              ...
              <div className="p-5">
                <div className="mb-2 flex items-center">
                  <div className="flex items-center space-x-1 text-amber-400">
                    {[1, 2, 3, 4, 5].map((star) => {
                      ...
                      if (rating >= star)
                        return <i key={star} className="las la-star"></i>;
                      if (rating >= star - 0.5)
                        return (
                          <i key={star} className="las la-star-half-alt"></i>
                        );
                ...
                <h6 className="mb-4 text-lg font-bold leading-tight text-slate-800 line-clamp-2 hover:text-emerald-500">
                  <Link
                    to={courseUrl}
                    className="text-decoration-none hover:no-underline text-slate-800 hover:text-emerald-500"
                  >
                    {data.tenKhoaHoc}
                  </Link>
                </h6>
                ...
                <div className="flex items-center justify-between text-sm font-medium text-gray-500">
                  ...
                  <Link
                    to={courseUrl}
                    className="flex items-center text-emerald-500 hover:text-emerald-600 text-decoration-none hover:no-underline"
                  >
