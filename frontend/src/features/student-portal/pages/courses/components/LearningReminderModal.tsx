...
export default function LearningReminderModal({ isOpen, onClose, onSuccess, currentCourseId, currentCourseName }: LearningReminderModalProps) {
  ...
  const handleGoogleCalendar = () => {
    const tieuDe = encodeURIComponent(form.tenNhacNho || "Nhắc nhở học tập");
    const chiTiet = encodeURIComponent("Lịch nhắc học tập được tạo từ hệ thống Edumeo");
    const googleCalendarUrl = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${tieuDe}&details=${chiTiet}`;
            ...
            <div className="space-y-6 animate-in fade-in">
              <div>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <button onClick={handleGoogleCalendar} className="inline-flex items-center justify-center gap-2 py-3 px-6 border-2 rounded-lg text-sm font-bold transition-colors hover:bg-slate-50" style={{ borderColor: '#5624D0', color: '#5624D0' }}>
