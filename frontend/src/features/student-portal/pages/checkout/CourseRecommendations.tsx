...
export default function CourseRecommendations({
  ...
  const idsParam =
    courseIds && courseIds.length > 0 ? courseIds.join(",") : courseId;
        ...
        if (response && response.recommendations) {
          ...
          if (response.crossSellVoucher) {
            localStorage.setItem(
              ...
              JSON.stringify({
                userId: userId,
                allowedCourseIds: response.recommendations.map(
                  (c: Course) => c.maKH,
                ),
          ...
          if (response.data.crossSellVoucher) {
            localStorage.setItem(
              ...
              JSON.stringify({
                userId: userId,
                allowedCourseIds: response.data.recommendations.map(
                  (c: Course) => c.maKH,
                ),
        ...
        {recommendations.map((course) => {
          ...
          if (crossSellVoucher) {
            if (crossSellVoucher.discountType === "PERCENT") {
              newPrice =
                course.giaBan -
                (course.giaBan * crossSellVoucher.discount) / 100;
            } else {
              ...
              {crossSellVoucher && (
                <div className="absolute top-3 left-0 z-10">
                  <div className="bg-gradient-to-r from-red-500 to-pink-500 text-white text-xs font-bold px-3 py-1.5 rounded-r-lg shadow-md transform -translate-x-1 group-hover:translate-x-0 transition-transform">
                    🔥 Giảm{" "}
                    {crossSellVoucher.discountType === "PERCENT"
                      ? `${crossSellVoucher.discount}%`
                      : `${crossSellVoucher.discount.toLocaleString("vi-VN")}đ`}
                    <div className="text-[10px] opacity-90 mt-0.5">
                      Mã: {crossSellVoucher.code}
                    </div>
                  </div>
                </div>
              )}
