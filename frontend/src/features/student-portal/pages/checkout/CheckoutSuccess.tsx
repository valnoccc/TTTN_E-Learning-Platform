...
import { MomoPaymentStatus, getInvoiceDetails, syncMomoReturn } from "../../../../api/checkout";
...
export default function CheckoutSuccess() {
  ...
  const paymentProvider = vnpTxnRef ? "VNPAY" : "MoMo";
  ...
  useEffect(() => {
    if (vnpTxnRef) {
      let cancelled = false;
      const invoiceId = Number(vnpTxnRef.split("_")[0]);
      if (!Number.isInteger(invoiceId)) {
        setReturnState("FAILED");
        setDisplayMessage("Mã giao dịch VNPAY không hợp lệ.");
      ...
      const checkVnpayStatus = async () => {
        if (vnpResponseCode && vnpResponseCode !== "00") {
          setReturnState("FAILED");
          setDisplayMessage("Giao dịch VNPAY không thành công.");
          return;
        }
        ...
        for (let attempt = 0; attempt < 6; attempt += 1) {
          try {
            ...
            setParsedCourseIds(details.map((item: any) => Number(item.MaKH)).filter(Number.isFinite));
            setParsedCourseId(Number(details[0]?.MaKH) || null);
            if (invoice?.TrangThaiThanhToan === "PAID") {
              setReturnState("PAID");
              setDisplayMessage("Thanh toán VNPAY thành công.");
              return;
            }
            if (invoice?.TrangThaiThanhToan === "FAILED" || invoice?.TrangThaiThanhToan === "CANCELLED") {
              setReturnState("FAILED");
              setDisplayMessage("Giao dịch VNPAY không thành công.");
              return;
            }
          } catch (error) {
            console.error("[CheckoutSuccess] Cannot check VNPAY invoice:", error);
          }
          await new Promise((resolve) => setTimeout(resolve, 1000));
        }
        if (!cancelled) {
          setReturnState("PENDING");
          setDisplayMessage("VNPAY đã nhận giao dịch. Hệ thống đang chờ xác nhận.");
        }
      };

      setReturnState("CHECKING");
      void checkVnpayStatus();
      return () => { cancelled = true; };
    }
  ...
  }, [message, resultCode, returnPayload, vnpResponseCode, vnpTxnRef]);
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h1 className="text-2xl font-semibold tracking-tight text-slate-900">
                  Hệ thống đang kiểm tra kết quả từ {paymentProvider}
                </h1>
              </div>
            </div>
            <p className="mt-4 max-w-xl text-sm leading-6 text-slate-600">
              Vui lòng chờ trong giây lát. Hóa đơn sẽ được cập nhật ngay khi
              giao dịch được đồng bộ xong.
            </p>
          </div>
        </div>
      </div>
            ...
            <div className="grid gap-0 lg:grid-cols-[1.15fr_0.85fr]">
              <div className="p-8 sm:p-10">
                ...
                <p className="mt-4 max-w-2xl text-base leading-7 text-slate-600">
                  {paymentProvider} đã xác nhận giao dịch và khóa học đã được thêm vào tài
                  khoản của bạn. Bạn có thể vào học ngay hoặc xem lại lịch sử
              ...
              <aside className="border-t border-stone-200 bg-stone-50/70 p-8 sm:p-10 lg:border-l lg:border-t-0">
                ...
                <dl className="mt-5 space-y-5">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <dd className="text-sm font-medium text-slate-900">{paymentProvider}</dd>
                  </div>
                  <div className="flex items-start justify-between gap-4">
                    <dt className="text-sm text-slate-500">Số khóa học</dt>
                    <dd className="text-sm font-medium text-slate-900">
                      {parsedCourseIds.length > 0
                        ? `${parsedCourseIds.length} khóa học`
                        : "Không xác định"}
                    </dd>
                  </div>
                  <div className="flex items-start justify-between gap-4">
        ...
        <div className="mx-auto flex min-h-[70vh] max-w-3xl items-center justify-center">
          <div className="w-full rounded-[28px] border border-stone-200 bg-white p-8 shadow-[0_20px_60px_rgba(15,23,42,0.08)] sm:p-10">
            ...
            <p className="mt-4 max-w-2xl text-sm leading-7 text-slate-600">
              {paymentProvider} chưa trả về trạng thái cuối cùng. Hóa đơn sẽ được cập nhật
              khi hệ thống nhận đủ kết quả xác nhận.
            </p>
            {displayMessage && (
  ...
  return (
    <div className="min-h-screen bg-stone-50 px-4 py-10">
      <div className="mx-auto flex min-h-[70vh] max-w-3xl items-center justify-center">
        <div className="w-full rounded-[28px] border border-stone-200 bg-white p-8 shadow-[0_20px_60px_rgba(15,23,42,0.08)] sm:p-10">
          <div className="inline-flex items-center gap-3 rounded-full border border-rose-200 bg-rose-50 px-4 py-2 text-sm font-medium text-rose-700">
          ...
          <p className="mt-4 max-w-2xl text-sm leading-7 text-slate-600">
            Thanh toán chưa hoàn tất và khóa học chưa được ghi danh vào tài
            khoản của bạn.
          </p>
          {displayMessage && (
