...
export default function Checkout() {
  ...
  const [formData, setFormData] = useState(getStoredCheckoutUser);
  const [paymentMethod, setPaymentMethod] = useState("MOMO");
  const [isProcessing, setIsProcessing] = useState(false);
  const [successData, setSuccessData] = useState<{ invoiceId: number } | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    if (
      ...
      location.state.selectedCourses.length > 0
    ) {
      ...
      if (isNaN(targetId)) {
        console.warn(
          ">>> [GUARD] Bỏ qua fetch khóa học vì courseId không hợp lệ (NaN):",
          courseId,
        );
  ...
  const autoApplyBestCoupon = async (courseIds: number[]) => {
    try {
      ...
      if (availableCoupons && availableCoupons.length > 0) {
        ...
        if (
          bestCoupon &&
          bestCoupon.isAvailable &&
          (bestCoupon.calculatedDiscount || 0) > 0
        ) {
            ...
            <Row>
              <Col lg={7}>
                ...
                <div className="card-box">
                  ...
                  {paymentMethod === "MOMO" ? (
                    <div
                      className="momo-details mt-4 p-4 rounded-lg"
                      style={{
                        background:
                          "linear-gradient(135deg, #fff0f6 0%, #ffe4f0 100%)",
                        border: "1.5px solid #d82d8b",
                      }}
                    >
                      <div className="d-flex align-items-center gap-3">
                        <img
                        ...
                        <div>
                          ...
                          >
                            Bấm <strong>"Xác nhận thanh toán"</strong> để được
                            chuyển đến trang thanh toán chính thức của MoMo
                            Sandbox. Quét mã QR hoặc xác nhận trên app.
                          </p>
                        </div>
                      </div>
                      <div
                        ...
                        style={{
                          background: "rgba(168, 0, 107, 0.07)",
                          fontSize: "13px",
                        }}
                      >
                        <span style={{ color: "#a5006b" }}>🔒</span>{" "}
                        <span style={{ color: "#555" }}>
                          Giao dịch được bảo mật bởi MoMo. Hệ thống sẽ tự động
                          ghi danh khóa học sau khi thanh toán thành công.
                        </span>
                      </div>
                    </div>
                  ) : null}
                    ...
                    <div className="bank-details mt-4">
                      ...
                      <div className="info">
                        ...
                        <p
                          ...
                          style={{ fontSize: "13px" }}
                        >
                          Vui lòng quét mã QR và ghi rõ số điện thoại của bạn
